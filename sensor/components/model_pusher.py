...
class ModelPusher:
    ...
    def __init__(self,model_pusher_config:ModelPusherConfig,
        ...
        try:
            logging.info(f"{'>>'*20} Model Transformation {'>>'*20}")
            self.model_pusher_config=model_pusher_config
            self.data_transformation_artifact=data_transformation_artifact
            self.model_trainer_artifact=model_trainer_artifact
            self.model_resolver=ModelResolver(model_resistry=self.model_pusher_config.saved_model_dir)
    ...
    def initiate_model_pusher(self)->ModelPusherArtifact:
        try:
            ...
            target_encoder_path = self.model_resolver.get_latest_save_target_encoedr_path()
